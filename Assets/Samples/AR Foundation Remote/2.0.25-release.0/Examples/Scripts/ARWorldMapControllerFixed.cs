#if ARKIT_INSTALLED && UGUI_INSTALLED
...
#endif
#endif // #if ARKIT_INSTALLED && UGUI_INSTALLED

namespace UnityEngine.XR.ARFoundation.Samples
{
    // AR_FOUNDATION_EDITOR_REMOTE: fix for Editor applied
    #if UNITY_EDITOR && ARKIT_INSTALLED && AR_FOUNDATION_REMOTE_INSTALLED
    ...
    /// </remarks>
    /// todoo ui size depends on screen resolution; also check other recordings
    public class ARWorldMapControllerFixed : MonoBehaviour
    {
        #if ARKIT_INSTALLED && UGUI_INSTALLED
        ...
        [ContextMenu(nameof(OnSaveButton))]
        /// <summary>
        ...
        [ContextMenu(nameof(OnLoadButton))]
        /// <summary>
        ...
        IEnumerator Load()
        {
            ...
            int bytesPerFrame = Application.isEditor ? 
                int.MaxValue : // load the whole file synchronously in the Editor for the code to be framerate-independent
                1024 * 10; // load the file in small pieces to prevent freezes on mobile devices in production
    ...
    #endif // ARKIT_INSTALLED && UGUI_INSTALLED 
    }
}
